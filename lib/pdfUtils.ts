  ...
  for (const element of elements) {
    try {
      if (element.type === "text") {
        ...
        const lines = pdf.splitTextToSize(textElement.content || "", textElement.width || 200);
        ...
        if (rotation !== 0) {
          ...
          if (shapeElement.shapeType === "rectangle") {
            ...
            if (strokeColor && strokeColor !== "transparent") {
              pdf.setLineWidth(shapeElement.strokeWidth || shapeElement.height || 1);
        ...
        const tableData = data.rows || Array(rows).fill(Array(columns).fill(""));
        ...
        if (Array.isArray(data)) {
          ...
          chartData = dataset.data.map((value, index) => ({
            ...
            color: dataset.backgroundColor || seriesColors[index % seriesColors.length],
          }));
        }
        ...
        if (chartType === "bar") {
          ...
          if (chartData.length > 1) {
            ...
            chartData.forEach((d, i) => {
              ...
              const pY = y + height - padding - ((d.value || 0) / maxValue) * innerHeight;
